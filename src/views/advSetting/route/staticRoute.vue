<template>
    <div>
        ...
        <h4>
            ...
            <span>{{ $lang(SWITCH_MAP[routeStatus]) }}</span>
            <el-button type="primary" size="small" @click="changeAdmin">{{
                $lang(BUTTON_TEXT[routeStatus])
            }}</el-button>
        </h4>
        <template v-if="!!routeStatus">
            ...
            <el-table :data="routeList" border>
                <el-table-column
                    :label="`${$lang('dst_ipaddr')} / ${$lang('mask')}`"
                >
                ...
                <el-table-column :label="$lang('protocol')" prop="protocol">
                    <template slot-scope="scope">{{
                        STATIC_ROUTE_PROTOCOL_MAP[scope.row.protocol]
                    }}</template>
                </el-table-column>
                <el-table-column
                ...
                <el-table-column :label="$lang('status')" prop="status">
                    <template slot-scope="scope">{{
                        STATIC_ROUTE_STATUS_MAP[scope.row.status]
                    }}</template>
                </el-table-column>
